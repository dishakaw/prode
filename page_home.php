<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PRODE</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.2/css/all.min.css">
    <link rel="stylesheet" href="levels.css">
</head>
<body>

    <div class="container">
        <nav class="flex">
            <div class="logo">PRODE</div>
            <ul>
                <li><a href="page_home.php">Home</a></li>
                <li><a href="aboutus.php">About Us</a></li>
                <li>
                    <a href="#">Levels
                        <i class="fas fa-caret-down"></i>
                    </a>
                <ul>
                   <li>
                       <a href="#">Rookie
                        <i class="fas fa-caret-right"></i>

                       </a>
                    <ul>
                     <li><a href="mcqsLevel1.php" target ="_self">level 1</a></li>
                     <li><a href="mcqsLevel2.php" target ="_self">level 2</a></li>
                     <li><a href="mcqsLevel3.php" target ="_self">level 3</a></li>
                    </ul>
                </li>
                <li>
                    <a href="#">Veteran 
                        <i class="fas fa-caret-right"></i>

                    </a>
                 <ul>
                  <li><a href="fillup.php" target ="_self">level 1</a></li>
                  <li><a href="fillup2.php" target ="_self">level 2</a></li>
                  <li><a href="fillup3.php" target ="_self">level 3</a></li>
                 </ul>
                </li>
                <li>
                    <a href="#">Master
                        <i class="fas fa-caret-right"></i>

                    </a>
                 <ul>
                  <li><a href="questionlevel1.php">level 1</a></li>
                  <li><a href="questionlevel2.php">level 2</a></li>
                  <li><a href="questionlevel3.php">level 3</a></li>
                 </ul>
                </li>

                
                  
                </ul>
            </li>
            <li><a href="page_login.php">LOG OUT</a></li>  
            </ul>
        </nav>
    </div>
    
</body>
</html>